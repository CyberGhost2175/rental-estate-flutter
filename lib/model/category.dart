...
Future<void> saveCategoryItems() async {
  try {
    ...
    final QuerySnapshot querySnapshot = await ref.get();


    for (final Category category in categoriesList) {
      DocumentReference docRef = await ref.add(category.toMap());
      print("Загружено в Firebase:");
      print("ID документа: ${docRef.id}");
      print("Данные: ${category.toMap()}");
      print("Данные для загрузки: ${category.toMap()}");
    }
    print("Данные успешно загружены в Firebase.");
  } catch (e) {
